...
function AddExpenseModal({ group, onClose, onSuccess }) {
  ...
  function handleOCR({ description, amount: a, billImage: b }) {
...
export default function GroupsPage() {
  ...
  // The group for which the expense modal is open (null = closed)
      ...
      {isMobile ? (
        <>
          ...
          {activeTab === 'create' && (
            <Card>
              <CardTitle>Create Group</CardTitle>
              <FormGroup label="Group Name"><Input placeholder="Goa Trip 2025" value={grpName} onChange={e => setGrpName(e.target.value)} /></FormGroup>
              <FormGroup label="Base Currency"><Select value={currency} onChange={e => setCurrency(e.target.value)}>{CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}</Select></FormGroup>
              <Alert message={createAlert?.msg} type={createAlert?.type} />
              <Button onClick={createGroup}>Create Group</Button>
            </Card>
          )}
          {activeTab === 'join' && (
            <Card>
              <CardTitle>Join Group</CardTitle>
              <FormGroup label="Invite Code"><Input placeholder="ABC123" value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())} style={{ letterSpacing: '3px' }} /></FormGroup>
              <Alert message={joinAlert?.msg} type={joinAlert?.type} />
              <Button onClick={joinGroup}>Join Group</Button>
            </Card>
          )}
...
// ── Group Card ────────────────────────────────────────────────────────────────
function GroupCard({ group: g, user, token, color, onAddExpense }) {
